...
RULE="-w /bin/kmod -p x -k modules"

echo "Backing up existing audit rules file to ${RULES_FILE}.bak"
sudo cp "$RULES_FILE" "${RULES_FILE}.bak" 2>/dev/null || true

echo "Checking if kmod audit rule already exists..."
if sudo grep -qF "$RULE" "$RULES_FILE" 2>/dev/null; then
    echo "Audit rule for kmod already present. No changes made."
else
    echo "Adding audit rule for kmod to $RULES_FILE"
...
echo "Audit rule for kmod applied and audit rules reloaded."
